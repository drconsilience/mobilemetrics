...
public class DeviceActivity extends ViewPagerActivity {
  ...
  private void updateSensorList() {
  	...
  	for (int i=0; i<TagSensor.SENSOR_LIST.length; i++) {
  		TagSensor sensor = TagSensor.SENSOR_LIST[i];
  ...
  boolean isEnabledByPrefs(final TagSensor sensor) {
  ...
  private void enableSensors(boolean enable) {
  	for (TagSensor sensor : mEnabledSensors) {
  	...
  	// Accelerometer calibration
        if (confUuid.equals(SensorTag.UUID_ACC_CONF) && enable) {
  		...
  		byte value =  enable ? sensor.getEnableSensorCode() : TagSensor.DISABLE_SENSOR_CODE;
 ...
 private void calibrateBarometer() {
	 ...
	 UUID servUuid = TagSensor.BAROMETER.getService();
	 UUID configUuid = TagSensor.BAROMETER.getConfig();
	 BluetoothGattService serv = mBtGatt.getService(servUuid);
	 ...
	 mBtLeService.writeCharacteristic(config,TagSensor.CALIBRATE_SENSOR_CODE);
	 mBtLeService.waitIdle(GATT_TIMEOUT);
		...
		if (mDeviceView != null) {
			...
			if (mHeightCalibrateRequest) {
				if (uuidStr.equals(SensorTag.UUID_BAR_DATA.toString())) {
					Point3D v = TagSensor.BAROMETER.convert(value);
