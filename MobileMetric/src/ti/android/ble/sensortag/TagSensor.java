...
public enum TagSensor {
  ...
  public Point3D convert(byte[] value) {
	 ...
	 * code.
	 */
  private TagSensor(UUID service, UUID data, UUID config, byte enableCode) {
   ...
   * */
  private TagSensor(UUID service, UUID data, UUID config) {
    this.service = service;
    this.data = data;
    this.config = config;
  ...
  public static TagSensor getFromDataUuid(UUID uuid) {
    for (TagSensor s : TagSensor.values()) {
  ...
  public static final TagSensor[] SENSOR_LIST = {IR_TEMPERATURE, ACCELEROMETER, MAGNETOMETER, GYROSCOPE, HUMIDITY, BAROMETER, SIMPLE_KEYS};
}
